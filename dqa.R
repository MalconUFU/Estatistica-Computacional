...
noticias <- read.csv("noticias.csv", header = TRUE, sep = ",")
...
noticias_tokens <- tokens(noticias$texto, remove_punct = TRUE, remove_symbols = TRUE, remove_numbers = TRUE, remove_url = TRUE) |> tokens_remove(stopwords("pt")) |> tokens_wordstem(language = "portuguese")

matriz_frequencia <- dfm(noticias_tokens)

n <- round(0.8 * nrow(noticias))

indices <- sample(x = 1:nrow(noticias), size = n, replace = FALSE)
